...
export function DayTourDetailPage({ tour: initialTour }: { tour: Tour }) {
  ...
  return <>
    ...
    <main className="tour-detail-page day-tour-detail container">
      <header className="tour-detail-header"><div><span className="eyebrow">{ar ? 'تجربة مصرية في يوم واحد' : 'One day Egypt experience'}</span><h1>{tour.title}</h1></div><button type="button" className={`tour-favorite ${favorite ? "active" : ""}`} onClick={() => favorites.toggle(tour.slug)} aria-label={ar ? 'احفظ الرحلة' : 'Save tour'} aria-pressed={favorite}><Heart size={20} fill={favorite ? "currentColor" : "none"} /></button></header>
